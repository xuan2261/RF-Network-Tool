...
if ($AppVersion -notmatch '^\d+\.\d+\.\d+$DataDir = if ($env:RFT_DATADIR -and (Test-Path -LiteralPath $env:RFT_DATADIR)) { $env:RFT_DATADIR } else { $BaseDir }
...
$btnScan.Add_Click({
    ...
    $scanProgress.Value=0;$lblScanSummary.Text="$($settings.Profile) | Online 0 | L2 0 | Total 0"
...
) { throw "VERSION không hợp lệ: $AppVersion" }
