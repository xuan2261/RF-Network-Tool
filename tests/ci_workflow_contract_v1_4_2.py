...
checks={
 ...
 'ui_sanitized_evidence':all(x in ui for x in ['physical-safe','physical-qualification-summary.json','ui-tab-items.txt','Remove raw physical-network evidence']),
 'ui_input_not_injected_into_run':'${{ inputs.interface_index }}' not in ui.split('run: |',1)[-1] and 'RFT_INTERFACE_INDEX: ${{ inputs.interface_index }}' in ui and "^[1-9]\\\\d*$" in ui,
