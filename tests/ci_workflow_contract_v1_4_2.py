...
checks={
 ...
 'ci_launcher_e2e':all(x in ci for x in ['WINDOWS_LAUNCHER_E2E_v1_4_2.ps1 -DiagnosticOnly','WINDOWS_LAUNCHER_E2E_v1_4_2.ps1 -AccessibilitySelfTest']),
 'ci_machine_cleanliness':'Machine cleanliness fail-closed qualification' in ci and 'WINDOWS_MACHINE_CLEANLINESS_TEST_v1_4_2.ps1' in ci,
 ...
 'ci_hosted_ui_noninteractive_only':ci.count('WINDOWS_LAUNCHER_E2E_v1_4_2.ps1')==2 and '-DiagnosticOnly' in ci and '-AccessibilitySelfTest' in ci,
 ...
 'ci_sbom_package_test':'release_sbom_test_v1_4_2.py' in ci,
 'ci_sbom_staged':all(x in ci for x in ['FULL-QA-CI-E2E-PROJECT.spdx.json','FULL-QA-CI-E2E-PORTABLE.spdx.json']),
 'ci_sbom_attested':ci.count('sbom-path:')==2 and ci.count('actions/attest@1e69f48acb82d1966a394da916b4c1698aa569d6')>=3,
 'ci_sbom_verified':'--predicate-type https://spdx.dev/Document/v2.3' in ci,
 ...
 'ci_release_artifacts_staged_in_workspace':all(x in ci for x in ['ci-artifacts/release/*.zip','ci-artifacts/release/*.spdx.json','Stage release candidates inside workspace']),
 ...
 'ci_release_assets':all(x in ci for x in ['gh release create','SHA256SUMS.txt','RF-Network-Tool-v${RELEASE_VERSION}-FULL-QA-CI-E2E-PORTABLE.zip','RF-Network-Tool-v${RELEASE_VERSION}-FULL-QA-CI-E2E-PROJECT.zip','RF-Network-Tool-v${RELEASE_VERSION}-FULL-QA-CI-E2E-PORTABLE.spdx.json','RF-Network-Tool-v${RELEASE_VERSION}-FULL-QA-CI-E2E-PROJECT.spdx.json']),
 'ci_release_checksum_covers_sbom':'sha256sum -- *.zip *.spdx.json' in ci and 'wc -l < SHA256SUMS.txt)\" -eq 4' in ci,
