...
checks={
 ...
 'ci_actionlint_pinned':all(x in ci for x in ['version=1.7.12','rhysd/actionlint/releases/download/v${version}','8aca8db96f1b94770f1b0d72b6dddcb1ebb8123cb3712530b08cc387b349a3d8','sha256sum -c -','/tmp/actionlint -color']),
 'ci_actionlint_retry':'curl --fail --location --silent --show-error --retry 3 --retry-delay 2' in ci,
 'ci_actionlint_no_pipe_installer':'| bash' not in ci and '| sh' not in ci,
 ...
 'ci_runner_cleanliness_smoke':all(x in ci for x in ['- name: Runner cleanliness smoke','if: always()','WINDOWS_RUNNER_CLEANLINESS_v1_4_2.ps1 -Phase Post','runner-cleanliness-${{ matrix.os }}.json']),
 ...
 'ci_release_job_tag_only':"name: Publish GitHub Release" in ci and "tags: [v1.4.2]" in ci and "if: github.event_name == 'push' && github.ref == 'refs/tags/v1.4.2'" in ci,
 ...
 'ci_release_assets':all(x in ci for x in ['gh release create','gh release upload','SHA256SUMS.txt','RF-Network-Tool-v1.4.2-FULL-QA-CI-E2E-PORTABLE.zip','RF-Network-Tool-v1.4.2-FULL-QA-CI-E2E-PROJECT.zip']),
 'ci_release_fail_closed':all(x in ci for x in ['Refusing to overwrite','release_sha','$GITHUB_SHA','RELEASE ASSET VERIFICATION PASSED']),
 ...
 'ui_runner_cleanliness_pre':all(x in ui for x in ['- name: Pre-run runner cleanliness','WINDOWS_RUNNER_CLEANLINESS_v1_4_2.ps1 -Phase Pre','runner-cleanliness-pre.json']),
 'ui_runner_cleanliness_post':all(x in ui for x in ['- name: Post-run runner cleanliness','WINDOWS_RUNNER_CLEANLINESS_v1_4_2.ps1 -Phase Post','runner-cleanliness-post.json']),
 'ui_runner_cleanliness_order':all(x in ui for x in ['- name: Exact revision guard','- name: Pre-run runner cleanliness','- name: Interactive desktop preflight','- name: Stage safe evidence and assert required physical gates','- name: Post-run runner cleanliness','- name: Remove raw physical-network evidence']) and ui.index('- name: Exact revision guard') < ui.index('- name: Pre-run runner cleanliness') < ui.index('- name: Interactive desktop preflight') and ui.index('- name: Stage safe evidence and assert required physical gates') < ui.index('- name: Post-run runner cleanliness') < ui.index('- name: Remove raw physical-network evidence'),
 'ui_runner_cleanliness_post_always':'- name: Post-run runner cleanliness\n        if: always()' in ui,
 ...
 'ui_structural_uniqueness':all(ui.count(x)==1 for x in ['- name: Exact revision guard','- name: Pre-run runner cleanliness','- name: Run full physical qualification','- name: Stage safe evidence and assert required physical gates','- name: Post-run runner cleanliness','- name: Upload sanitized physical evidence']),
