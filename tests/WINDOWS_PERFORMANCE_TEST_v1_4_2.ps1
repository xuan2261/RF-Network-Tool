...
try{
  ...
  if(Test-Path $state){
    ...
    $hasIpv6Error=$null -ne $s.metrics.PSObject.Properties['IPv6NeighborError']
    ...
    Assert-True $hasIpv6Error 'FAST synthetic /24 exposes IPv6 neighbor error metric'
    ...
    $report.scan=[ordered]@{targets=[int]$s.total;elapsedMs=$elapsed;budgetMs=$ScanBudgetMs;online=[int]$s.online;seen=[int]$s.seen;ipv6Neighbors=$(if($hasIpv6Snapshot){@($s.ipv6Neighbors).Count}else{0});ipv6NeighborError=$(if($hasIpv6Error){[string]$s.metrics.IPv6NeighborError}else{''});effectivePingConcurrency=[int]$s.metrics.EffectivePingConcurrency;effectiveArpConcurrency=[int]$s.metrics.EffectiveArpConcurrency}
  }
